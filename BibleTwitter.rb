...
class BibleTwitter
    ...
    def performSearch()
        return @client.search("bible OR quote OR love OR jesus OR strength OR inspirational OR #bible OR #citationneededbible OR #thecitationneededbible -filter:retweets", result_type: "recent", count: 10)
    end
    ...
    def self.filterSearch(search)
        ...
        search.each do |tweet|
            filtered = self.filterTweet(tweet)
            if !filtered.nil?
                out << filtered
            end
        end
